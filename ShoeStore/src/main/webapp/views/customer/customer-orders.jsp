...
<head>
    ...
    <meta name="description" content="View your complete purchase history at ADIDIS. Track order status, total amounts, and order dates." />
    ...
    <style>
        ...
        .row-fade-in { animation: fadeInRow 0.25s ease forwards; }
        @keyframes fadeInRow {
...
<main class="ml-64 flex-1 flex flex-col min-h-screen">
    ...
    <header class="px-margin-desktop pt-margin-desktop pb-8 flex justify-between items-end border-b border-outline-variant/30">
        <div>
            <h2 class="font-headline-lg text-headline-lg text-primary tracking-tight">Order History</h2>
        ...
        <div class="flex gap-4">
            <span class="font-label-sm text-label-sm text-secondary uppercase tracking-widest self-end pb-1">
    ...
    <section class="px-margin-desktop py-6 flex justify-between items-center bg-surface-bright sticky top-0 z-30 shadow-[0_1px_0_rgba(0,0,0,0.05)]">
        <form method="get" action="${pageContext.request.contextPath}/customer/orders" class="flex items-center gap-4 flex-1">
                            ...
                            <tr>
                                <td colspan="4" class="py-20 px-6 text-center text-secondary">
                                    <div class="flex flex-col items-center gap-4">
                                        <span class="material-symbols-outlined text-[64px] text-outline-variant">receipt_long</span>
                                        <p class="font-label-md text-label-md uppercase tracking-wider">You have no orders yet</p>
                                        <a href="${pageContext.request.contextPath}/home"
                        ...
                        <c:otherwise>
                            <c:forEach var="order" items="${orders}" varStatus="loop">
                                <tr class="hover:bg-surface-container-low transition-colors group row-fade-in"
                                    ...
                                    <td class="py-4 px-6">
                                        <span class="inline-flex items-center px-2 py-1 font-label-sm text-label-sm uppercase tracking-widest rounded-none status-${order.status}">
                                            <c:choose>
                        ...
                        <c:forEach var="p" begin="1" end="${totalPages}">
                            <c:choose>
                                <c:when test="${p == currentPage}">
                                    <span class="w-8 h-8 flex items-center justify-center bg-primary text-on-primary font-label-sm text-label-sm rounded-none">${p}</span>
                                </c:when>
                                <c:otherwise>
