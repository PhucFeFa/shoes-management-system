                ...
                <div class="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-24 mb-16">
                        ...
                        <div class="flex flex-col gap-6 self-start">
                            <div class="flex items-center justify-center bg-surface-container-high rounded-3xl p-12">
                                <img onerror="this.onerror=null;this.src='${pageContext.request.contextPath}/assets/fallback.png';"
                                    src="${product.firstImageUrl}" alt="${product.name}"
                                    class="w-full h-auto object-contain mix-blend-multiply">
                            </div>

                            <!-- Description -->
                            <div class="mt-6 pt-6 border-t border-outline-variant/30">
                                <h3 class="text-headline-md font-headline-md font-bold text-primary mb-4">Description
                                </h3>
                        ...
                        <div class="flex flex-col py-8">
                            ...
                            <!-- Title -->
                            <h1
                                class="text-display-lg-mobile md:text-display-lg font-display-lg font-bold uppercase text-primary tracking-tight leading-none mb-6">
                                ${product.name}
                            </h1>
                            ...
                            <c:choose>
                                <c:when test="${not empty reviews}">
                                    ...
                                    <div class="flex items-center gap-2 mb-8 border-b border-outline-variant/30 pb-8">
                                        <div
                                            class="relative inline-block text-outline-variant text-xl tracking-widest mr-2 whitespace-nowrap">
                                            ★★★★★
                            ...
                            <div class="space-y-8 mb-8 border-b border-outline-variant/30 pb-8">
                                ...
                                <div>
                                    ...
                                    <div class="grid grid-cols-3 sm:grid-cols-4 gap-3" id="sizeContainer">
                                        <c:forEach var="size" items="${sizes}">
                                            <button type="button"
                                                class="size-btn border border-outline-variant rounded-lg py-3 px-4 font-label-md text-label-md uppercase text-primary hover:border-primary transition-colors focus:ring-2 focus:ring-primary focus:outline-none"
                                                data-size="${size}">
                                                ${size}
                                            </button>
                                        </c:forEach>
                                ...
                                <div>
                                    <label
                                        class="block font-headline-md text-headline-md font-bold text-primary mb-4">Select
                                        Color</label>
                                    <div class="flex flex-wrap gap-3" id="colorContainer">
                                        <c:forEach var="color" items="${colors}">
                                            <button type="button"
                                                class="color-btn border border-outline-variant rounded-lg px-6 py-3 font-label-md text-label-md uppercase text-primary hover:border-primary transition-colors focus:ring-2 focus:ring-primary focus:outline-none"
                                                data-color="${color}">
                                                ${color}
                                            </button>
                                        </c:forEach>
                            ...
                            <c:choose>
                                <c:when
                                    test="${not empty sessionScope.currentUser && (sessionScope.currentUser.roleName eq 'Admin' || sessionScope.currentUser.roleName eq 'Staff')}">
                                    <div
                                        class="bg-error-container text-on-error-container p-4 rounded-2xl mb-4 font-body-md text-body-md">
                                        Admin/Staff accounts are only permitted to view products, not to make purchases.
                                    </div>
                                    <button
                                        class="w-full bg-surface-container-highest text-on-surface-variant py-5 rounded-full font-label-md text-label-md uppercase tracking-wider cursor-not-allowed opacity-50"
                                        disabled>
                                        Read Only View
                                    </button>
                                </c:when>
                                <c:otherwise>
                                    <form action="${pageContext.request.contextPath}/AddToCart" method="post"
                                        ...
                                        <button id="addToCartBtn" type="submit"
                                            class="w-full bg-primary text-on-primary py-5 rounded-full font-label-md text-label-md uppercase tracking-wider hover:opacity-90 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                                            disabled>
                                            Add to Cart
                                        </button>
                                    </form>
                    ...
                    <div class="w-full mt-16 pt-16 border-t border-outline-variant/30">
                                        ...
                                        <c:choose>
                                            <c:when test="${not myReview.updated}">
                                                <form action="${pageContext.request.contextPath}/review" method="POST">
                                                    ...
                                                    <div class="mb-4">
                                                        <label class="block font-semibold mb-2">Comment</label>
                                                        <textarea id="updateComment" name="comment" rows="3" required
                                                            maxlength="200" class="w-full border rounded-lg px-4 py-2"
                                            ...
                                            <c:otherwise>
                                                <div
                                                    class="bg-surface-container-low p-6 rounded-3xl border border-outline-variant mb-4">
                                                    <p class="text-body-md text-secondary mb-2">
                                                        <c:out value="${myReview.comment}" />
                                                    </p>
                                                    <p
                                                        class="text-label-sm font-label-sm text-outline-variant uppercase tracking-widest mb-6">
                                                        You have already
                            ...
                            <div class="bg-gray-50 p-6 rounded-lg mb-8 border text-center text-gray-500">
                                <c:choose>
                                    <c:when test="${not empty sessionScope.currentUser}">
                                        <p class="text-sm"><i class="bi bi-info-circle me-2"></i>Only customers with
                                            completed/delivered orders for this product can leave a review.</p>
                                    </c:when>
                                    <c:otherwise>
                                        <p class="text-sm"><i class="bi bi-info-circle me-2"></i>Please <a
                                                href="${pageContext.request.contextPath}/login"
                                                class="underline font-semibold text-primary">log in</a> with an account
                                            that has a completed order to leave a review.</p>
                                    </c:otherwise>
                                </c:choose>
                            </div>
                        </c:if>
                        ...
                        <c:if test="${not empty reviews}">
                            <div class="mb-12">
                                <h3 class="text-headline-sm font-headline-sm font-bold text-primary mb-6">Filter by
                                    Rating</h3>
                                <div class="flex flex-wrap gap-3">
                                    ...
                                    <button
                                        ...
                                        data-rating="5">
                                        <span class="flex items-center gap-[2px]"><span
                                                class="material-symbols-outlined text-[#eab308] text-[20px]"
                                                ...
                                                style="font-variation-settings: 'FILL' 1;">star</span><span
                                                class="material-symbols-outlined text-[#eab308] text-[20px]"
                                                style="font-variation-settings: 'FILL' 1;">star</span></span> (5)
                                    </button>
                                    <button
                                        class="review-filter-btn px-6 py-3 border border-outline-variant rounded-full text-label-md font-label-md text-primary hover:border-primary transition-colors flex items-center gap-2 active:scale-95"
                                        data-rating="4">
                                        <span class="flex items-center gap-[2px]"><span
                                                ...
                                                class="material-symbols-outlined text-[#eab308] text-[20px]"
                                                style="font-variation-settings: 'FILL' 1;">star</span><span
                                                class="material-symbols-outlined text-outline-variant text-[20px]">star</span></span>
                                        (4)
                                    </button>
                                    <button
                                        class="review-filter-btn px-6 py-3 border border-outline-variant rounded-full text-label-md font-label-md text-primary hover:border-primary transition-colors flex items-center gap-2 active:scale-95"
                        ...
                        <div class="space-y-8">
                            <c:choose>
                                <c:when test="${empty reviews}">
                                    <p class="text-body-md text-secondary italic text-center py-8">No reviews yet. Be
                                        the first to review!</p>
                                </c:when>
                                <c:otherwise>
                                    <c:forEach var="r" items="${reviews}">
                                        <div class="review-item bg-surface-container-low p-8 rounded-3xl"
                                            ...
                                            <div class="flex items-center justify-between mb-4">
                                                ...
                                                <span class="flex items-center gap-[2px]">
                                                    ...
                                                    <c:forEach begin="${r.rating + 1}" end="5">
                                                        <span
                                                            class="material-symbols-outlined text-outline-variant text-[20px]">star</span>
                                                    </c:forEach>
                                                </span>
                                            </div>
                                            ...
                                            <c:if test="${r.updated}">
                                                <div
                                                    ...
                                                    <p
                                                        class="text-label-sm font-label-sm text-secondary mb-2 uppercase tracking-widest">
                                                        Previous
                                                        Comment:</p>
                                            ...
                                            <c:if test="${not empty r.replyComment}">
                                                <div
                                                    class="mt-4 p-5 bg-primary/5 border-l-4 border-primary rounded-2xl">
                                                    <div class="flex items-center justify-between mb-1">
                                                        <p
                                                            class="text-label-md font-label-md text-primary uppercase tracking-widest flex items-center gap-2">
                                                            <span
                                                                class="material-symbols-outlined text-[18px]">support_agent</span>
                                                            <c:out
                                                                value="${not empty r.replierName ? r.replierName : 'Store'}" />
                                                        </p>
                                                        <c:if test="${not empty r.replyUpdatedAt}">
                                                            <span class="text-label-sm font-label-sm text-secondary">
                                                                <fmt:formatDate value="${r.replyUpdatedAt}"
                                            ...
                                            <div
                                                class="flex flex-col sm:flex-row justify-between items-start sm:items-center mt-6 gap-2">
                                                ...
                                                <c:if test="${r.updated}">
                                                    <span
                                                        class="text-label-sm font-label-sm text-primary italic">Updated:
